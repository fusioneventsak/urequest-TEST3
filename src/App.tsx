...
function App() {
  ...
  useEffect(() => {
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      ...
      // Handle DOMException AbortError specifically
      if (event.reason instanceof DOMException && event.reason.name === 'AbortError') {
        // Silently handle AbortError from fetch operations
        event.preventDefault();
        return;
      }
      
      // Also check for generic abort errors
      if (errorMessage.includes('aborted') || 
          errorMessage.includes('Component unmounted') ||
          errorMessage.includes('channel closed')) {
        // Silently handle these errors
  ...
  const handleCreateSetList = useCallback(async (newSetList: Omit<SetList, 'id'>) => {
    ...
    try {
      ...
      if (data && songs && songs.length > 0) {
        ...
        const songMappings = songs.map((song,