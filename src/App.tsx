...
function App() {
  ...
  const handleResetQueue = useCallback(async () => {
    ...
    try {
      ...
      if (pendingCount === 0) {
        toast.info('No pending requests to clear');
        return;
      }
      ...
      toast.success(`Queue cleared successfully (${pendingCount} requests removed)`);
    } catch (error) {
      ...
      if (error instanceof Error && (
        ...
        toast.error('Network error while clearing queue. Please check your connection and try again.');
      } else {
