 ...
 * Specifically designed for critical updates like song locking and queue clearing
 */
export function useUltraFastRequestSync(onUpdate: (requests: SongRequest[]) => void) {
  ...
  const handleRealtimeEvent = useCallback((payload: any) => {
    console.log('🔥 Real-time event:', payload.eventType, payload.table);
    
    // Critical events get immediate processing (no delays)
    const isCritical = 
      payload.eventType === 'INSERT' || // New requests
      payload.new?.is_locked !== payload.old?.is_locked || // Lock changes
      payload.new?.is_played !== payload.old?.is_played; // Queue clears
    
    if (isCritical) {
      console.log('🚨 CRITICAL - immediate fetch');
      fetchRequests('critical');
    } else {
      // Non-critical events get slight delay
      setTimeout(() => fetchRequests('normal'), 50);
    }
  }, [fetchRequests]);
  ...
  }, [handleRealtimeEvent, fetchRequests]);
